    ...
    <div className="w-full h-full pt-12 overflow-y-scroll scrollbar-hide">
      {data?.map((room) => (
        <div
          ...
          key={room.roomId}
          className={`relative w-full h-20 bg-slate-100 rounded-md mt-4 px-4 flex items-center shadow-sm ${
            isChatLoading
              ? "cursor-not-allowed"
        ...
        >
          ...
          {isChatLoading && (
            <div className="absolute rounded-md left-0 right-0 top-0 bottom-0 w-full h-full bg-white/40 flex justify-center items-center">
